...
5.) Once done then default auth config will get created on the location vim /Users/user/.config/gcloud/application_default_credentials.json
6.) Open IAM & Admin service, create new service account and add pub-sub producer and pub-sub subscriber role for the same
7.) Create keys for this newly create service account, json file will get download. Copy the content of that json file and replace it under /Users/user/.config/gcloud/application_default_credentials.json
